...
class _MarkdownExtractor:
    def __init__(self, paragraphs: list, images_dir: Path | None = None) -> None:
    ...
    def _process_paragraph(self) -> None:
        paragraph = self.paragraphs[self.idx]
        image_refs: list[str] = []
        if self.images_dir is not None:
            image_refs, self.image_counter = _extract_paragraph_images(paragraph, self.images_dir, self.image_counter)
        ...
        if not text:
            self._process_empty_text(formula_text)
            return
        ...
        if formula_text:
            self._process_formula(formula_text)
            return
        ...
        self._process_text_line(text, list_kind)
    ...
    def _process_images(self, image_refs: list[str], text: str) -> bool:
        ...
        if image_caption is None:
            for look_ahead in range(self.idx + 1, min(len(self.paragraphs), self.idx + 6)):
                next_text = self.paragraphs[look_ahead].text.strip()
                if not next_text:
                    continue
    ...
    def _process_empty_text(self, formula_text: str | None) -> None:
        self._end_list()
        if formula_text:
            self._flush_code_buffer()
    ...
    def _process_formula(self, formula_text: str) -> None:
        self._end_list()
        self._flush_code_buffer()
        self.lines.append(f"$${formula_text}$$")
        self.lines.append("")
    ...
    def _process_text_line(self, text: str, list_kind: str | None) -> None:
...
def _extract_body_markdown(paragraphs: list, start_idx: int, images_dir: Path | None = None) -> str:
    ...
    extractor = _MarkdownExtractor(body_paragraphs, images_dir)
