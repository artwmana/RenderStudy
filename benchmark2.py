...
# To actually trigger the block, we need the conversion_service.convert_input_file to run and take time.
# The convert_input_file is CPU bound and blocks the event loop.
...
async def simulate():
    ...
    # Create dummy MD file that takes some time to parse
    with tempfile.NamedTemporaryFile(suffix=".md", delete=False) as tf:
        content = "# Title\n\n" + "Some paragraph text with formatting.\n\n" * 1000
        tf.write(content.encode('utf-8'))
        tf_path = tf.name

    # We mock out just the telegram updates to let the real convert_input_file run.
